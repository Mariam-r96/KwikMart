...
const CategoryBasedProducts = (props) => {
    ...
    useEffect(() => {
        const storage_cartItems = JSON.parse(localStorage.getItem("cart")) ? JSON.parse(localStorage.getItem("cart")) : [];

        axios.get(`https://shodai.herokuapp.com/api/category/${query.productCategory ? query.productCategory : ''}`)
        .then( response => {
